...
Roco.prototype.run = function (cmd, callback) {
    ...
    roco.hosts.forEach(function (host) {
        ...
        if ('object' === typeof roco.connections[host]) {
            connection = roco.connections[host];
        } else {
            var config = {
                host: host,
                port: port
            };
            config = readSSHConfig(config);
            connection = roco.connections[host] = new Connection(config);
        }
...
function readSSHConfig(config) {
    if (!sshConfigFile) {
        try {
            sshConfigFile = fs.readFileSync(process.env.HOME + '/.ssh/config');
        } catch(e) {
            sshConfigFile = '';
        }
    }

    var hostIndex = sshConfigFile.indexOf('Host ' + host);
    if (hostIndex === -1) {
        hostIndex = sshConfigFile.indexOf('HostName ' + host);
    }
    if (hostIndex === -1) {
        return config;
    }

    var relevant = sshConfigFile.substr(hostIndex).split(host + '\n    ')[1];
    relevant = relevant.split('Host')[0];

    var port = extract('Port');
    var username = extract('User');
    var host = extract('HostName');
    var privateKey = extract('IdentityFile');
    if (privateKey) {
